...
#
# WICHTIG: existiert die Datei bereits, wird NUR der Admin-Eintrag ersetzt und
# alles andere unverändert übernommen. Vorher hat dieses Skript die Datei
# gelöscht und mit genau einem Nutzer neu geschrieben – wer von Hand weitere
# Nutzer angelegt hatte, verlor sie beim nächsten `regen-secrets` (und
# scripts/ensure-secrets.sh ruft dieses Skript automatisch auf, sobald eine der
# erzeugten Dateien fehlt). Der Hinweis im Kopf der erzeugten Datei war
# außerdem in beide Richtungen falsch: eigene Einträge überlebten nicht, und
# eine Admin-UI zum Anlegen von Nutzern hat das file-Backend von Authelia nicht.
ADMIN_BLOCK="$(cat <<EOF
  ${AUTHELIA_ADMIN_USER}:
    disabled: false
...
PYEOF
    ...
    # kann apphost sie nicht überschreiben (EACCES), wohl aber im
    # apphost-eigenen secrets/-Ordner löschen und neu anlegen.
    rm -f "$OUTPUT_USERS"
